function params=fd(doPlot,varargin);
 ...
 pcpct=0.99; nkern=11; fname=[];
 parseAssignVarargs({'pcpct','nkern','fname'});

% read in design
 inf=['design.txt' fname];
 design=textread(inf); m=size(design,1);
% scale design
 xmin=min(design); xrange=max(design)-xmin;
 design=(design-repmat(xmin,m,1))./repmat(xrange,m,1);

% read in sim data
 inf=['sim_outputs' fname];
 simdata=textread(inf);
 ysim=simdata(:,2:end);
 tsim=simdata(:,1);
...
% read in obs data
 ...
 for ii=1:n
   inf=['obs_outputs' int2str(ii) fname];
...
% compute on simulations
 ...
 if(doPlot)
   ...
   figure(1); print('-depsc2',['fdPc' fname]); close;
 end
...
% plot sims and data
 if (doPlot)
   ...
   figure(2); print('-depsc2',['fdDat' fname]); close;
 end
