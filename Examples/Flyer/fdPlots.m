...
% Set up parameter labels
 xlabs={};
 thlabs={'\epsilon','\theta_0','\kappa','\gamma','y_0',...
         'y_\infty','s_0','s_\infty','P_{min}','v_s'};
 labs=[xlabs thlabs];
...
% Set up prediction grid
 ...
 if doPlot(1)
   ...
   for ii=1:pu;
     ...
     text(-0.4,0.5,['PC' int2str(ii)],'FontSize',12,...
 ...
 if doPlot(4)
   ...
   for ii=1:lsub
     ...
     h(ii)=gPackSubplot(ceil(lsub/np),np,ceil(ii/np),mod(ii-1,np)+1,0.7);
     plot3(repmat(pout.simData.orig.t,size(grid)),...
           repmat(tt,size(pout.simData.orig.t)),r','b'); view(AzEl);
     xlabel('time'); ylabel(labs(jj)); zlabel('velocity');
     alpha(0.25);
     axis tight;
