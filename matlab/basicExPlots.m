  ...
  if any(doPlot([5 6]))
    ...
    if doPlot(5);     % do the scaled space plot
      ...
      %    in the standardized space
        py=pyd+pye;
        pym =mean(py ); pylim =gQuantile(py ,[0.5 0.95]);
        pyem=mean(pye); pyelim=gQuantile(pye,[0.5 0.95]);
        pydm=mean(pyd); pydlim=gQuantile(pyd,[0.5 0.95]);
...
% uncertaints overlaid.
  if doPlot(9)
    ...
    % take the mean and limits, and transform back to original space
      ...
      ypl=(simData.Ksim*squeeze(gQuantile(pred.w,.5,1)) )';
